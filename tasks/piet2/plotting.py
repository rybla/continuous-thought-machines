...
def draw_path(x, route, valid_only=False, gt=False, cmap=None):
    """
    Draws a path on a piet2 image based on a given route.

    Args:
        x: A numpy array representing the piet2 image.
    ...
    Returns:
        A numpy array representing the piet2 image with the path drawn in blue.
    """
    x = np.copy(x)
...
def make_piet2_gif(
    inputs, predictions, targets, attention_tracking, save_location, verbose=True
):
    """
    ...
    figscale = 1
    aspect_ratio = (
        len(mosaic[0]) * figscale,
        len(mosaic) * figscale * img_aspect,
    )  # W, H
    ...
    with tqdm(
        ...
        if verbose:
            pbar.set_description("Processing frames for piet2 plotting")
        for stepi in np.arange(0, predictions.shape[-1], 1):
            fig, axes = plt.subplot_mosaic(mosaic, figsize=aspect_ratio)  # type: ignore
            for ax in axes.values():
                ax.axis("off")
            guess_piet2 = draw_path(
                np.moveaxis(inputs, 0, -1), predictions.argmax(1)[:, stepi], cmap=cmap
            )
            ...
            aggregated_attention = (
                torch.nn.functional.interpolate(
                    torch.from_numpy(attention_now).unsqueeze(0),
                    inputs.shape[-1],
                    mode="bilinear",
                )[0]
                .mean(0)
            ...
            aggregated_attention[
                aggregated_attention < np.percentile(aggregated_attention, 80)
            ] = 0.0
            route_steps.append(find_center_of_mass(com_attn))  # type: ignore
            ...
            mapped_attention = (mapped_attention - mapped_attention.min()) / np.ptp(
                mapped_attention
            )
            # np.clip(guess_piet2 * (1-mapped_attention[...,np.newaxis]*0.5) + (cmap_viridis(mapped_attention)[:,:,:3] * mapped_attention[...,np.newaxis])*1.3, 0, 1)
            overlay_img = np.clip(
                guess_piet2 * (1 - mapped_attention[..., np.newaxis] * 0.6)
                + (
                    cmap_viridis(mapped_attention)[:, :, :3]
                    * mapped_attention[..., np.newaxis]
                )
                * 1.1,
                0,
                1,
            )  # np.clip((np.copy(guess_piet2)*(1-aggregated_attention[:,:,np.newaxis])*0.7 + (aggregated_attention[:,:,np.newaxis]*3 * np.reshape(np.array(colour)[:3], (1, 1, 3)))), 0, 1)
            ...
            axes["route"].imshow(
                np.flip(np.moveaxis(inputs, 0, -1), axis=0), origin="lower"
            )
            # ax.imshow(np.flip(solution_piet2, axis=0), origin='lower')
            arrow_scale = 2
            ...
            image_numpy = image_numpy.reshape(*reversed(canvas.get_width_height()), 4)[
                :, :, :3
            ]  # Get RGB
            ...
            # # frame = np.clip((np.copy(guess_piet2)*0.5 + (aggregated_attention[:,:,np.newaxis] * np.reshape(np.array(colour)[:3], (1, 1, 3)))), 0, 1)
