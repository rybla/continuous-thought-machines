...
def make_piet1_gif(
    ...
    with tqdm(
        ...
        for stepi in np.arange(0, predictions.shape[-1], 1):
            ...
            # np.clip(guess_piet1 * (1-mapped_attention[...,np.newaxis]*0.5) + (cmap_viridis(mapped_attention)[:,:,:3] * mapped_attention[...,np.newaxis])*1.3, 0, 1)
            overlay_img = np.clip(
                guess_piet1 * (1 - mapped_attention[..., np.newaxis] * 0.6)
                + (
                ...
                * 1.1,
                0,
                1,
            )  # np.clip((np.copy(guess_piet1)*(1-aggregated_attention[:,:,np.newaxis])*0.7 + (aggregated_attention[:,:,np.newaxis]*3 * np.reshape(np.array(colour)[:3], (1, 1, 3)))), 0, 1)
            ...
            # ax.imshow(np.flip(solution_piet1, axis=0), origin='lower')
            arrow_scale = 2
            ...
            # # frame = np.clip((np.copy(guess_piet1)*0.5 + (aggregated_attention[:,:,np.newaxis] * np.reshape(np.array(colour)[:3], (1, 1, 3)))), 0, 1)
